...
<head>
  <meta charset=\"utf-8\" />
  <title>Due Diligence Report - {property_id}</title>
  <style>
...
<body>
  ...
  <div class=\"meta\">Property ID: {property_id} | Generated: {datetime.utcnow().isoformat()}</div>

  <div class=\"card\">
    <h2>Final Assessment</h2>
    <pre>{json.dumps(final, ensure_ascii=True, indent=2)}</pre>
  </div>

  <div class=\"card\">
    <h2>Legal Analysis</h2>
    <pre>{json.dumps(legal, ensure_ascii=True, indent=2)}</pre>
  </div>

  <div class=\"card\">
    <h2>Risk Analysis</h2>
    <pre>{json.dumps(risk, ensure_ascii=True, indent=2)}</pre>
  </div>

  <div class=\"card\">
    <h2>Valuation Analysis</h2>
    <pre>{json.dumps(valuation, ensure_ascii=True, indent=2)}</pre>
  </div>

  <div class=\"card\">
    <h2>Metadata</h2>
    <pre>{json.dumps(metadata, ensure_ascii=True, indent=2)}</pre>
  </div>
</body>
</html>
"""
    ...
    def generate_report(
        ...
        try:
            html = self._render_html(property_id, analysis_data, metadata)
